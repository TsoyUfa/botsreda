...
async def update_existing_database():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if 'last_activity' not in column_names:
            await cursor.execute("ALTER TABLE users ADD COLUMN last_activity TIMESTAMP")
        ...
        if 'experience_level' not in column_names:
            await cursor.execute("ALTER TABLE users ADD COLUMN experience_level TEXT")
        ...
        # Устанавливаем значения по умолчанию для last_activity
        if 'last_activity' in column_names:
            await cursor.execute("""
                UPDATE users 
                SET last_activity = COALESCE(last_activity, registration_date)
            """)
            print("✅ Обновлены значения last_activity")
        
        # Обрабатываем is_premium - создаем новую таблицу без этой колонки
        if 'is_premium' in column_names:
            # Проверяем, нужно ли перестраивать таблицу
            has_premium = True
            try:
                await cursor.execute("SELECT is_premium FROM users LIMIT 1")
            except:
                has_premium = False
            ...
            if has_premium:
                ...
                # Копируем данные
                columns_to_copy = []
                for col in ['user_id', 'username', 'first_name', 'registration_date', 'last_activity', 
                           'last_name', 'phone', 'email', 'agency', 'experience_level', 'is_active', 
                           'total_time_spent', 'total_sessions']:
                    if col in column_names or col == 'last_name':  # last_name может быть новой
                        columns_to_copy.append(col)
                
                columns_str = ', '.join(columns_to_copy)
                await cursor.execute(f"""
                    INSERT INTO users_temp ({columns_str})
                    SELECT {columns_str} FROM users
                """)
                
                # Удаляем старую таблицу и переименовываем новую
                await cursor.execute("DROP TABLE users")
                await cursor.execute("ALTER TABLE users_temp RENAME TO users")
        ...
        indexes = [
            ("CREATE INDEX IF NOT EXISTS idx_users_activity ON users (last_activity)", "idx_users_activity"),
            ("CREATE INDEX IF NOT EXISTS idx_activity_user_timestamp ON user_activity_log (user_id, timestamp)", "idx_activity_user_timestamp"),
            ("CREATE INDEX IF NOT EXISTS idx_progress_user_module ON user_progress (user_id, module_id)", "idx_progress_user_module"),
            ("CREATE INDEX IF NOT EXISTS idx_quiz_user_module ON quiz_results (user_id, module_id)", "idx_quiz_user_module"),
            ("CREATE INDEX IF NOT EXISTS idx_sessions_user ON learning_sessions (user_id)", "idx_sessions_user"),
            ("CREATE INDEX IF NOT EXISTS idx_daily_stats_date ON daily_stats (date)", "idx_daily_stats_date")
        ]
        ...
        for sql, name in indexes:
            try:
                ...
                print(f"⚠️ Индекс {name} уже существует или ошибка: {e}")
